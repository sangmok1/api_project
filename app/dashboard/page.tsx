...
export default function DashboardPage() {
  ...
  const saveQRCode = () => {
    ...
    if (savedQRCodes.length >= 50) {
      setSaveMessage("최대 50개까지만 저장할 수 있습니다.")
    ...
    saveToLocalStorage(updatedQRCodes)
                ...
                {qrCode && (
                  <div className="flex flex-col items-center space-y-4 mt-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 items-center">
                      <Button onClick={saveQRCode} variant="secondary">
                        QR 코드 저장
                      </Button>
                      <DownloadButton
                        svgRef={qrCodeRef}
              ...
              <TabsContent value="saved" className="space-y-4">
                ...
                {filteredQRCodes.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    {savedQRCodes.length === 0 ? "저장된 QR 코드가 없습니다." : "검색 결과가 없습니다."}
                  </div>
                ) : (
                  <div className="grid gap-4">
                    {filteredQRCodes.map((savedQR) => {
                      return (
                        <Card key={savedQR.id} className="p-4">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            <div className="flex flex-col sm:flex-row gap-4">
                              <QRCodeSVG
                                ref={(el) => {
                                  if (el) qrRefs.current[savedQR.id] = el
                                }}
                                value={savedQR.url}
                                size={80}
                                fgColor={savedQR.color}
                                bgColor={savedQR.backgroundColor}
                                level={savedQR.errorCorrection}
                                includeMargin={true}
                              />
                              <div className="flex-1 space-y-2">
                                <div className="flex items-center gap-2">
                                  <LinkIcon className="h-4 w-4" />
                                  <span className="font-medium break-all">{savedQR.url}</span>
                                </div>
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                  <Calendar className="h-4 w-4" />
                                  <span>{savedQR.createdAt}</span>
                                </div>
                                <div className="text-sm text-muted-foreground">
                                  Size: {savedQR.size}x{savedQR.size} | Error Correction: {savedQR.errorCorrection}
                                </div>
                              </div>
                            </div>
                            <div className="flex flex-row sm:flex-col gap-2 self-end sm:self-start">
                              <DownloadButton
                                svgRef={() => qrRefs.current[savedQR.id]}
                                url={savedQR.url}
                                size={savedQR.size}
                                fileName={`qrcode-${savedQR.id}`}
                              />
                              <Button
                                variant="destructive"
                                size="sm"
